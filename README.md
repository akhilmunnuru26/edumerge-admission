...
A: "The assignment's core challenge is data integrity. PostgreSQL provides ACID transactions essential for preventing race conditions in seat allocation. The database trigger validates quota availability atomically, which would require complex application-level locking in MongoDB."

**Q: How do you prevent double-booking?**

A: "Three-layer defense:
1. Database transaction (BEGIN...COMMIT)
2. BEFORE INSERT trigger checks seats_filled < allocated_seats
3. Row-level locking on quota_seat_usage table

Even with 10 simultaneous requests for the last seat, PostgreSQL serializes them. Only 1 succeeds."

**Q: What are performance bottlenecks?**

A: "Potential issues:
1. Dashboard queries joining 5 tables → Solution: Use materialized views or Redis caching
2. Admission number sequence → Solution: Sequence caching (already built-in to PostgreSQL)
3. Concurrent allocations → Current solution handles ~500 TPS, beyond that we'd need connection pooling (already implemented) and read replicas"
